...
class Widgets():#class for creating a objects like timer ect.
    ...
    def endGame(self):
        ...
        if((len(self.Results)>1)):
            self.avgAccuracy=round(self.avgAccuracy/(len(self.Results)))#Calculating round average Accuracy
            self.avgCountWords=round(self.avgCountWords/(len(self.Results)))#Calculating round average Count Words
        self.res.close()
